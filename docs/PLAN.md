...
Regras:
- `format`: `short` → 30–165s de conteúdo (alvo média ~60s; teto reserva ~15s p/ intro+vinheta → final ≤180s), 1080x1920, sem crop (vídeo inteiro sobre fundo blur), legendas queimadas; `corte` → 480–600s (8–10 min, ≥8 min p/ monetização), 1920x1080, sem burn, com moldura de marca preto+amarelo + CTA de inscrição (`branding.py`).
