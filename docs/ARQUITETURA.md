...
- **Whisper**: `WhisperModel("large-v3", device="cuda", compute_type="int8")` — GTX 1660 SUPER: `int8`, nunca fp16 (NaN na série 16xx). Fallback CPU int8 + modelo `small`. `word_timestamps=True, vad_filter=True, beam_size=5`. DLLs CUDA: `os.add_dll_directory(site-packages/nvidia/{cublas,cudnn}/bin)` no Windows.
  - **Batched (default)**: `BatchedInferencePipeline(model).transcribe(..., batch_size=4)` — chunks-VAD em paralelo na GPU (~1.6× sobre o stream single-pass; medido 5.3×→8.4× realtime num clip real). Reusa o modelo já carregado (sem VRAM extra de pesos). Força `condition_on_previous_text=False` → menos alucinação/repetição em áudio longo. `batch_size 4` = pico ~5.5GB/6GB; guarda de OOM halva o batch e recarrega. `--batch-size 1` = stream legado; CPU ignora batching.
  - **Diarização concorrente**: `diarize_turns` (CPU) depende só do áudio, então roda numa `ThreadPoolExecutor` em paralelo ao Whisper (GPU) quando o device efetivo é `cuda`; wall-clock = `max(whisper, diarize)` em vez de `whisper+diarize`. Verificado byte-idêntico ao sequencial. **A diarização é o piso** (~0.24× a duração do áudio; thread-invariante, medido — `DIARIZE_SEG_THREADS`/`DIARIZE_EMB_THREADS` existem mas não aceleram). No fallback CPU roda sequencial (evita contenda).
  - **Turbo (opt-in)**: `--model deepdml/faster-whisper-large-v3-turbo-ct2 --batch-size 8` — ~21× realtime, VRAM ~3.3GB; timestamps ~2-3× menos precisos → só quando a precisão do corte/legenda não importa.
