...
4. **Formato:** decida short vs corte vs ambos pela regra de decisão da referência (payoff em <=45s do hook -> short; bloco temático de 2–10 min com 2+ picos -> corte; momento excepcional score >= 85 -> ambos).
...
- `short`: `render.crop = "center"`, `render.target_resolution = "1080x1920"`, `captions.burn = true`.
- `corte`: `render.crop = "none"`, `render.target_resolution = "1920x1080"`, `captions.burn = false`.

Campos de copy (`title`, `title_alts`, `description`, `tags`) ficam null/vazios — são do copywriter, não seus. `rejected_notable` com 3–5 itens para o humano auditar suas decisões.
